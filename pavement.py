...
try:
    from ddug_geonode.settings import GEONODE_APPS
except:
...
def start_geoserver(options):
    ...
    from ddug_geonode.settings import OGC_SERVER
...
def _reset():
    sh("rm -rf ddug_geonode/development.db")
    sh("rm -rf ddug_geonode/uploaded/*")
    _install_data_dir()
